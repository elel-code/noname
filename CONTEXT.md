...
2. 核心算法集中在 `algorithms.py`：`Ingredient`/`CandidateSolution` 表征混合染色体，后者使用位串（bytearray）存储选择掩码 + `array('f')` 存配比，以降低成千上万成分时的内存占用；`vector_to_candidate` 将 `[0,1]` 向量映射为选中状态与配比，评估流程计算 Loewe 协同、SwissADME 肝毒性与基于配比的多样性惩罚。
